...
// Override of ExpandNode to inspect the selection pin value during blueprint compilation
void UK2Node_SwitchOnFloatRanges::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
    UEdGraphPin* SelectionPin = GetSelectionPin();
    if (SelectionPin)
    {
        ...
        if (SelectionNet)
        {
            UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("EXPANDING NODE - Selection pin connected to net: %s"), *SelectionNet->PinName.ToString());
            
            // Look at the selection pin's value during compilation
            FString PinValue = SelectionPin->DefaultValue;
            UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("  - Selection pin default value: %s"), *PinValue);
            ...
            if (SelectionPin->LinkedTo.Num() > 0)
            {
                ...
                UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("  - Linked from pin: %s on node %s"), 
                ...
                if (VarNode)
                {
                    ...
                    {
                        UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("    => Blueprint: %s"), *BP->GetName());
                        ...
                        if (BP->GeneratedClass)
                        {
                            ...
                            if (VarProperty)
                            {
                                UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("    => Found variable property: %s"), *VarProperty->GetName());
                            }
                            else
                            {
                                UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("    => Could not find variable property"));
                            }
                        }
                    }
                }
            ...
            {
                UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("  - Selection pin has no incoming links"));
            }
        }
        else
        {
            UE_LOG(LogSwitchOnFloatRanges, Warning, TEXT("EXPANDING NODE - Selection pin not connected to any net"));
        }
    }
