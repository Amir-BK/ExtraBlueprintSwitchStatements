...
public:
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		...
		TArray<FBlueprintCompiledStatement*> CaseLabels;
		for (UScriptStruct* PinStruct : SwitchNode->PinStructs)
		{
			if (PinStruct)
			{
				FBlueprintCompiledStatement* Label = &Context.AppendStatementForNode(Node);
				Label->Type = KCST_Nop;
				CaseLabels.Add(Label);
			}
		}

		// Handle each case with conditional branching
		for (int32 PinIdx = 0; PinIdx < SwitchNode->PinStructs.Num(); PinIdx++)
		{
			...
			// Find the corresponding execution pin for this struct type
			...
			BranchStatement.LHS = SelectionTerm;

			if (PinIdx < CaseLabels.Num())
			{
...
void UK2Node_SwitchInstancedStructType::CreateCasePins()
{
	...
	for (int32 Index = 0; Index < PinStructs.Num(); ++Index)
	{
		...
		CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Struct, PinStructs[Index], PinNames[Index]);
